...
write_delim(equiv_effect_summer_temps, 
            file.path(out_dir, experiment, "Summaries", "air_temp_equiv_summerSWT.txt"),
            delim = "\t")
...
write_delim(equiv_effect_winter_temps, 
            file.path(out_dir, experiment, "Summaries", "air_temp_equiv_winterSWT.txt"),
            delim = "\t")
...
abs_temps <- abs_change_seasonal %>%
  select(season, T_change, Q_change, surfaceT, schmidt) %>%
  mutate(T_change = as.numeric(as.character(T_change)),
         Q_change = as.numeric(as.character(Q_change))) %>%
  arrange(T_change, Q_change)
...
write_delim(mit_summer_temps, file.path(out_dir, experiment, "Summaries", "mitigation_potential_summerSWT.txt"),
            delim = "\t")
...
write_delim(mit_winter_temps, file.path(out_dir, experiment, "Summaries", "mitigation_potential_winterSWT.txt"),
            delim = "\t")
